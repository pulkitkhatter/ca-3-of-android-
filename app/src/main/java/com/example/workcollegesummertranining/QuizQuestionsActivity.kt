...
class QuizQuestionsActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        backButton.setOnClickListener {
            if (mCurrentPosition > 1) {
                mCurrentPosition-- // Move to the previous question
                setQuestion()
            }else {
    ...
    private fun setQuestion() {
        val questionNumberText = "Question $mCurrentPosition of ${mQuestionsList!!.size}"
        tvQuestionNumber?.text = questionNumberText
        ...
        if (mCurrentPosition == mQuestionsList!!.size) {
            ...
            buttonSubmit?.text = if (question.isAnswerSubmitted) "NEXT QUESTION" else "SUBMIT"
        }

        if (question.isAnswerSubmitted) {
            disableOptions() // Disable options if answer is submitted
            ...
            enableOptions() // Enable options if answer is not submitted
        }
        ...
        when (view?.id) {
            ...
            R.id.btn_submit->{
                ...
                if (mSelectedOptionPosition == 0) {
                    ...
                    if (mCurrentPosition > mQuestionsList!!.size) {
                        // Launch the ResultActivity with the user's score
                        val intent = Intent(this, ResultActivity::class.java)
                        intent.putExtra("USER_SCORE", mCorrectAnswers)
